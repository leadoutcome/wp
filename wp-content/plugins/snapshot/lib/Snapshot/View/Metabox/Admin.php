<?php
if ( ! class_exists( "Snapshot_View_Metabox_Admin" ) ) {
	class Snapshot_View_Metabox_Admin {
		 ...
		 * @return self
		 */
		function Snapshot_View_Metabox_Admin() {
			$this->__construct();
		}
					...
					<table class="form-table snapshot-backup-header">
						...
						<?php
							...
							if ( is_multisite() ) {
									...
									<tr class="form-field">
										...
										<td>
											<input type="hidden" name="snapshot-blog-id" id="snapshot-blog-id"
											       value="<?php echo $current_blog->blog_id; ?>"/>
								...
								?><input type="hidden" id="snapshot-blog-id" name="snapshot-blog-id"
								         value="<?php echo $wpdb->blogid ?>" /><?php
							}
						}
						?>
													...
													<tr class="form-field">
														...
														<td>
															<?php
																$user_name = '';
																if (isset($item['user']))
																	$user_name = Snapshot_Helper_Utility::get_user_name(intval($item['user']));
																echo $user_name;
															?>
														</td>
													</tr>
							...
							<td>
								...
								<ul>
									<li><input type="radio" class="snapshot-tables-option"
											...
											<?php if ( $item['tables-option'] == "none" ) {
									...
									<li><input type="radio" class="snapshot-tables-option"
											...
											<?php if ( $item['tables-option'] == "selected" ) {
												echo ' checked="checked" ';
											} ?>
									           name="snapshot-tables-option"> <label
										...
										<div id="snapshot-selected-tables-container"
											...
											<div id="snapshot-tables-<?php echo $table_set_key ?>-set"
											     ...
											     style="display: <?php echo $display_set; ?>">

												<p class="snapshot-tables-title"><?php echo $table_set_title; ?> <?php if ( $table_set_key != 'error' ) { ?>
														<a
												...
												<ul class="snapshot-table-list"
												    id="snapshot-table-list-<?php echo $table_set_key; ?>">
													<?php
													if ( ( isset( $table_sets[ $table_set_key ] ) ) && ( count( $table_sets[ $table_set_key ] ) ) ) {
														...
														foreach ( $tables as $table_key => $table_name ) {
															...
															if ( $table_set_key == 'error' ) {
																...
																?>
																<li><input type="checkbox" <?php echo $is_checked; ?>
																           class="snapshot-table-item"
																           id="snapshot-tables-<?php echo $table_key; ?>"
																           value="<?php echo $table_key; ?>"
																           name="snapshot-tables[<?php echo $table_set_key; ?>][<?php echo $table_key; ?>]">
																<label
							...
							<td class="left">
								...
								<ul>
									...
									if ( ! empty( $blog_upload_path ) ) {
										?>
										<li><input type="radio" class="snapshot-files-option"
										           id="snapshot-files-option-all" value="all"
												<?php if ( $item['files-option'] == "all" ) {
													echo ' checked="checked" ';
												} ?>
										           name="snapshot-files-option"> <label
									...
									<?php if ( ( ! isset( $item['blog-id'] ) ) || ( is_main_site( $item['blog-id'] ) ) ) { ?>
										<li class="snapshot-backup-files-sections-main-only"><input type="radio"
										                                                            ...
										                                                            id="snapshot-files-option-selected"
										                                                            value="selected"
												<?php if ( $item['files-option'] == "selected" ) {
													echo ' checked="checked" ';
												} ?>
											...
											<div id="snapshot-selected-files-container"
											     ...
											     if ( ( $item['files-option'] == "none" ) || ( $item['files-option'] == "all" ) ) {
												     echo ' display:none; ';
											     } ?>">
...
<?php */ ?>
													...
													<li><input type="checkbox" class="snapshot-backup-sub-options"
															<?php if ( array_search( 'plugins', $item['files-sections'] ) !== false ) {
																echo ' checked="checked" ';
															} ?>
													...
													<li><input type="checkbox" class="snapshot-backup-sub-options"
															<?php if ( array_search( 'media', $item['files-sections'] ) !== false ) {
																echo ' checked="checked" ';
															} ?>
													...
													<li><input type="checkbox" class="snapshot-backup-sub-options"
															<?php if ( array_search( 'config', $item['files-sections'] ) !== false ) {
																echo ' checked="checked" ';
															} ?>
													...
													<li><input type="checkbox" class="snapshot-backup-sub-options"
															<?php if ( array_search( 'htaccess', $item['files-sections'] ) !== false ) {
																echo ' checked="checked" ';
															} ?>
								...
								<div id="snapshot-selected-files-sync-container">
									...
									<ul>
										...
										<li><input type="radio" name="snapshot-destination-sync"
										           id="snapshot-destination-sync-archive" value="archive"
										           class="snapshot-destination-sync"
										...
										<li><input type="radio" <?php echo $_is_mirror_disabled; ?>
										           name="snapshot-destination-sync"
										           id="snapshot-destination-sync-mirror" value="mirror"
										           class="snapshot-destination-sync"
						...
						<tr class="form-field">
							...
							<td>
								<select name="snapshot-destination" id="snapshot-destination">
									...
									Snapshot_Helper_UI::destination_select_options_groups(
										WPMUDEVSnapshot::instance()->config_data['destinations'],
										$item['destination'],
										WPMUDEVSnapshot::instance()->get_setting( 'destinationClasses' ) ) ?>
								</select><br/>
						...
						<tr class="form-field">
							...
							<td>
								<input type="text" name="snapshot-destination-directory"
								       ...
								       value="<?php if ( isset( $item['destination-directory'] ) ) {
								...
								if ( $snapshot_destination_selected_type == 'google-drive' ) {
									...
									<ul class="description">
										...
										<li>
											...
											?></li>
										<li>
											<strong>[SNAPSHOT_ID]</strong> &ndash; <?php _e( 'This is the unique ID assigned to this Snapshot set' ); ?>
											:
											<?php
					...
					<table class="form-table snapshot-backup-migration">
						...
						foreach ( $config_data['items'] as $item ) {
							...
							<tr class="form-field <?php echo $row_class; ?>">
								...
								<td><?php
									$backupFile = trailingslashit( WPMUDEVSnapshot::instance()->get_setting( 'backupBaseFolderFull' ) ) . $item['file'];

									if ( file_exists( $backupFile ) ) {

										?><a
										href="<?php echo trailingslashit( WPMUDEVSnapshot::instance()->get_setting( 'backupURLFull' ) ) .
					...
					<div class="postbox">
						...
						<div class="inside">
							...
							<tr class="form-field">
								...
								<?php
									if (isset($item['blog-id']))
										$site_tables = Snapshot_Helper_Utility::get_database_tables($item['blog-id']);
									else
										$site_tables = Snapshot_Helper_Utility::get_database_tables();
				...
				<div class="inside">
					...
					if ( ( isset( $item['data'] ) ) && ( count( $item['data'] ) ) ) {
						...
						foreach ( $data_items as $data_key => $data_item ) {
							...
							$backupLogFileFull = trailingslashit( WPMUDEVSnapshot::instance()->get_setting( 'backupLogFolderFull' ) )
							...
							?>
							<td><?php echo Snapshot_Helper_Utility::show_date_time( $data_item['timestamp'] ); ?></td><?php
							...
							<td>
								...
								if ( $themes ) {
									...
									foreach ( $themes as $theme_key => $theme_name ) {
										...
										?>
										<li><input type="radio"
										           id="snapshot-restore-option-theme-<?php echo $theme_key; ?>"
												<?php
											...
											<label for="snapshot-restore-option-theme-<?php echo $theme_key; ?>">
												<?php if ( $theme_name == $current_theme ) {
													echo '<strong>';
												} ?>
												<?php echo $theme_name ?>
												<?php if ( $theme_name == $current_theme ) {
					...
					<table class="form-table snapshot-restore-blog-options">
						...
						if ( ( is_multisite() ) && ( $blog_details->siteurl != $item['MANIFEST']['WP_SITEURL'] ) ) {
							...
							$restore_blog_id = $item['blog-id'];
						           ...
						           value="<?php echo $restore_blog_id; ?>"/>
						</td>
						</tr>
						<tr>
							<td style="width: 50%">
										...
										<span id="snapshot-blog-search-error" style="color: #FF0000; display:none;"><?php
											...
											<br/></span>
												...
												if ( is_subdomain_install() ) {
													...
													?><input name="snapshot-blog-id-search" id="snapshot-blog-id-search"
													         value="<?php echo $current_sub_domain; ?>"
													...
													?><?php echo untrailingslashit( network_site_url() ) . $current_path; ?><input
					...
					<tr class="form-field">
						...
						<td>
							<input type="text" name="backupFolder" id="snapshot-settings-backupFolder"
							       value="<?php echo WPMUDEVSnapshot::instance()->config_data['config']['backupFolder']; ?>"/>
							...
							<?php
								$response_object = wp_remote_get( trailingslashit( WPMUDEVSnapshot::instance()->get_setting( 'backupURLFull' ) ) );
				...
				<table class="form-table snapshot-settings-segment-size">
					<tr class="form-field">
						...
						<td>
							...
							echo WPMUDEVSnapshot::instance()->config_data['config']['segmentSize']; ?>"/>
			...
			<table class="form-table snapshot-settings-server-info">
				...
				<tr class="form-field">
					...
					<td>
						<?php
						$folders_array = array(
							WPMUDEVSnapshot::instance()->get_setting( 'backupBaseFolderFull' ),
							WPMUDEVSnapshot::instance()->get_setting( 'backupBackupFolderFull' ),
							WPMUDEVSnapshot::instance()->get_setting( 'backupRestoreFolderFull' ),
							WPMUDEVSnapshot::instance()->get_setting( 'backupLockFolderFull' ),
							WPMUDEVSnapshot::instance()->get_setting( 'backupLogFolderFull' )
						);
						sort( $folders_array );
						...
						<tr class="form-field">
							...
							<td>
								<input type="text" name="memoryLimit" id="snapshot-settings-memory-limit"
								       value="<?php echo WPMUDEVSnapshot::instance()->config_data['config']['memoryLimit']; ?>"/>
				...
				<table class="form-table snapshot-settings-segment-size">
					<tr class="form-field">
						...
						<textarea name="filesIgnore" id="filesIgnore" cols="20" rows="5"><?php
							if ( ( isset( WPMUDEVSnapshot::instance()->config_data['config']['filesIgnore'] ) )
							     && ( is_array( WPMUDEVSnapshot::instance()->config_data['config']['filesIgnore'] ) )
							     && ( count( WPMUDEVSnapshot::instance()->config_data['config']['filesIgnore'] ) )
							) {
								echo implode( "\n", WPMUDEVSnapshot::instance()->config_data['config']['filesIgnore'] );
							}
							?></textarea>
							...
							<p class="description"><?php echo sprintf( __( 'The Snapshot directory %s is automatically excluded from all snapshots',
									...
									'<code>' . trailingslashit( WPMUDEVSnapshot::instance()->get_setting( 'backupBaseFolderFull' ) ) . '</code>' ); ?>
						</td>
					</tr>
					<tr>
						<td>&nbsp;</td>
					...
					foreach ( $error_reporting_errors as $error_key => $error_label ) {
						...
						<tr>
							<td>
								...
								if ( isset( WPMUDEVSnapshot::instance()->config_data['config']['errorReporting'][ $error_key ]['stop'] ) ) {
									echo ' checked="checked" ';
								}
								?> />
							</td>
					...
					<tr>
						...
						<td>
							...
							if ( class_exists( 'ZipArchive' ) ) {
								if ( ! isset( WPMUDEVSnapshot::instance()->config_data['config']['zipLibrary'] ) ) {
									WPMUDEVSnapshot::instance()->config_data['config']['zipLibrary'] = 'ZipArchive';
								}
								?>
								<input type="radio" name="zipLibrary" id="snapshot-settings-zip-library-ziparchive"
								       value="ZipArchive" <?php
								if ( WPMUDEVSnapshot::instance()->config_data['config']['zipLibrary'] == 'ZipArchive' ) {
									echo ' checked="checked" ';
								} ?> /> <?php
							...
							<?php
								...
								if ( ! isset( WPMUDEVSnapshot::instance()->config_data['config']['zipLibrary'] ) ) {
									WPMUDEVSnapshot::instance()->config_data['config']['zipLibrary'] = 'PclZip';
								}
							}
							?>
							<input type="radio" name="zipLibrary" id="snapshot-settings-zip-library-ziparchive"
										...
										<div id="snapshot-selected-tables-container"
											...
											<div id="snapshot-tables-<?php echo $table_set_key ?>-set"
												...
												if ( ( isset( $item['data'][ $data_item_key ]['tables-sections'][ $table_set_key ] ) )
													...
													?>
													<ul class="snapshot-table-list"
													    id="snapshot-table-list-<?php echo $table_set_key; ?>"><?php
													foreach ( $tables as $table_key => $table_name ) {
...
//			echo "data_item<pre>"; print_r($data_item); echo "</pre>";
										...
										<div id="snapshot-selected-files-container"
											...
											<ul id="snapshot-select-files-option">
												<?php if ( array_search( 'themes', $item['data'][ $data_item_key ]['files-sections'] ) !== false ) { ?>
													<li id="snapshot-files-option-themes-li"><input type="checkbox"
													                                                class="snapshot-backup-sub-options"
													                                                checked="checked"
													                                                id="snapshot-files-option-themes"
													                                                value="themes"
												...
												<?php if ( array_search( 'plugins', $item['data'][ $data_item_key ]['files-sections'] ) !== false ) { ?>
													<li id="snapshot-files-option-plugins-li"><input type="checkbox"
													                                                 class="snapshot-backup-sub-options"
													                                                 checked="checked"
													                                                 id="snapshot-files-option-plugins"
													                                                 value="plugins"
												...
												<?php if ( array_search( 'media', $item['data'][ $data_item_key ]['files-sections'] ) !== false ) { ?>
													<li id="snapshot-files-option-media-li"><input type="checkbox"
													                                               class="snapshot-backup-sub-options"
													                                               checked="checked"
													                                               id="snapshot-files-option-media"
													                                               value="media"
												...
												<?php if ( array_search( 'config', $item['data'][ $data_item_key ]['files-sections'] ) !== false ) { ?>
													<li id="snapshot-files-option-config-li"><input type="checkbox"
													                                                class="snapshot-backup-sub-options"
													                                                id="snapshot-files-option-config"
													                                                value="config"
												...
												<?php if ( array_search( 'htaccess', $item['data'][ $data_item_key ]['files-sections'] ) !== false ) { ?>
													<li id="snapshot-files-option-htaccess-li"><input type="checkbox"
													                                                  class="snapshot-backup-sub-options"
													                                                  id="snapshot-files-option-htaccess"
													                                                  value="htaccess"
		...
		function snapshot_metaboxes_show_destination_items() {
			//echo "config_data<pre>"; print_r(WPMUDEVSnapshot::instance()->config_data); echo "</pre>";
			?>
			<form action="?page=snapshots_settings_panel" method="post">
			<input type="hidden" name="snapshot-action" value="settings-update"/>
				...
				<div class="inside">
					<ul>
						...
						<li>
							...
							<p><?php echo sprintf( __( 'For archives already on your server but not showing in the All Snapshots listing you can simply submit this form without entering a value below. This will scan the snapshot archivs directory (<em>%s</em>) for any missing archives and add them to the listing.', SNAPSHOT_I18N_DOMAIN ), WPMUDEVSnapshot::instance()->get_setting( 'backupBaseFolderFull' ) ); ?></p>
						...
						<tr>
							<td>
								...
								<input type="text" style="width: 90%;" id="snapshot-import-archive-remote-url"
								       name="snapshot-import-archive-remote-url" value="<?php echo $remote_file; ?>"/>
							</td>
						</tr>
					</table>
