...
class Snapshot_View_Full_Backup {
	...
	public function render_page () {
		$page = false;
		if (!current_user_can($this->get_page_role())) return false;

		if (!$this->_model->has_dashboard()) $page = 'get_started';
		else if (!$this->_model->is_active()) $page = 'activate_backup';
		else {
			$page = 'backups';
	...
	public function render_full_destination () {
		if (!current_user_can('manage_snapshots_destinations')) return '';
