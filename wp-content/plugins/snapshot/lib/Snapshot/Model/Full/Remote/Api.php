...
class Snapshot_Model_Full_Remote_Api extends Snapshot_Model_Full {
	...
	public function get_token () {
		...
		if (empty($key)) {
			$this->_set_error(sprintf(Snapshot_View_Full_Backup::get_message('missing_secret_key'), $this->get_current_secret_key_link()));
		...
		if (200 !== (int)wp_remote_retrieve_response_code($response)) {
			...
			$this->_set_error($error);
			if (!empty($args['token'])) Snapshot_Helper_Log::warn("Unable to exchange token", "Remote");
			else Snapshot_Helper_Log::warn("Unable to get fresh token", "Remote");
			return false;
		}
	...
	public function get_dev_api_unprotected_response ($endpoint, $body=array()) {
		...
		if (!in_array($endpoint, array('get-urls', 'backups-size'))) return new WP_Error(
			$this->get_filter('invalid_endoint'),
			__('Invalid endpoint', SNAPSHOT_I18N_DOMAIN)
		);
