...
class Snapshot_Model_Full_Backup extends Snapshot_Model_Full_Abstract {
	...
	public function has_errors () {
		if ($this->_storage->has_errors()) return true;
		if ($this->_local->has_errors()) return true;
	...
	public function is_dashboard_installed () {
		if ($this->is_dashboard_active()) return true;
	...
	public function get_frequencies () {
		return apply_filters(
			$this->get_filter('schedule_frequencies'),
			array(
	...
	public function get_schedule_times () {
		...
		for ($i = 0; $i < DAY_IN_SECONDS; $i += HOUR_IN_SECONDS) {
			...
			if ($seconds >= DAY_IN_SECONDS) $seconds -= DAY_IN_SECONDS;
			if ( 0 == $seconds ) $seconds = 1; // Because 0 will show current time in Hub :(
