...
class VOP_HtmlFormParser {
	 ...
	 */
	function VOP_HtmlFormParser( $html_data ) {
	...
	function parseForms() {
		if ( preg_match_all("/<form.*>.+<\/form>/isU", $this->html_data, $forms) ) {
			foreach ( $forms[0] as $form ) {
				...
				if ( preg_match_all("/<input\s+type=[\"']?hidden[\"']?.*>/iU", $form, $hiddens) ) {					
				 ...
				 */
				if ( preg_match_all("/<input.*type=[\"']?email[\"']?.*>/iU", $form, $texts) ) { 
