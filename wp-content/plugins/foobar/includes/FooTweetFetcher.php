...
if ( !class_exists( 'FooTweetFetcher' ) ) {
	...
	class FooTweetFetcher {
		...
		const URL_USER_TIMELINE = 'statuses/user_timeline';
		const URL_SEARCH = 'search/tweets';
		...
		public function get_tweets_for_user($username, $args) {
			$transient_key = 'foobar_tweets-' . $username . '-tweets';

			if ( ($data = get_transient( $transient_key )) === false ) {
				//transient has expired - fetch fresh tweets
				$data = $this->get_tweets_from_api( self::URL_USER_TIMELINE, $this->build_params_for_user($username, $args) );
		...
		public function get_tweets_by_search($query, $args) {
			$transient_key = 'foobar_tweets-' .$query . '-tweets';

			if ( ($data = get_transient( $transient_key )) === false ) {
				//transient has expired - fetch fresh tweets
				$data = $this->get_tweets_from_api( self::URL_SEARCH, $this->build_params_for_search($query, $args) );
		...
		private function build_params_for_search($query, $args) {
			//setup params
			$params = array();
			$params['q'] = strip_tags( sanitize_user( $query ) );
			if ( array_key_exists('limit', $args) ) {
				$params['count'] = intval( $args['limit'] );
			}
			if ( array_key_exists('result_type', $args) ) {
				$params['result_type'] = $args['result_type'];
			}
			return $params;
		}

		private function build_params_for_user($username, $args) {
			//setup params
			$params                = array();
		...
		private function get_tweets_from_api($url, $params) {
			...
			$response = $twitter_oauth->get( $url, $params );
