...
define('FOOBAR_FILE_JS', 'jquery.foobar.2.4.min.js');
...
//          ) );
		...
		function init() {
			...
			add_action('wp_head', array($this, 'add_admin_bar_css'), 9);
...
//          }
		...
		function foobar_save_meta($post_id) {
			...
			if (array_key_exists('foobar_nonce', $_POST) &&
				...
				$is_default = array_key_exists('foobar_is_default', $_POST) ? $_POST['foobar_is_default'] : false;
...
//        }
    ...
    <td>
		...
		function render_foobar_twitter_meta_box_content($post) {
			...
			$twitterHashtag      = $this->get_meta($data, "twitterHashtag", "");
			$twitterMaxTweets = $this->get_meta($data, "twitterMaxTweets", "5");
			$twitterCacheHours = $this->get_meta($data, "twitterCacheHours", "5");
			?>
			<table class="form-table">
				<tbody>
				<tr>
				...
				<tr>
					<td class="first-column">Twitter Hashtags</td>
					<td>
						<input name="foobar[twitterHashtag]" type="text" value="<?php echo $twitterHashtag; ?>"/>
						<br/>
						<small>Include tweets with this hashtag</small>
					</td>
					<td>
				</tr>
				<tr>
				...
				<tr>
					<td class="first-column">Tweet Cache Time (hours)</td>
					<td>
						<input name="foobar[twitterCacheHours]" class="foobar-num-slider" data-slidewidth="80px"
							   type="text" value="<?php echo $twitterCacheHours; ?>"/>
						<br/>
						<small>The amount of time that tweets are cached.</small>
					</td>
					<td>
				</tr>
			</table>
		...
		function frontend_has_foobar($post) {
			...
			if (!apply_filters('show_foobar', true)) return false;
			...
			if ( $this->get_option('foobar_disable_for_mobile') == 'on' &&
				($this->_mobile->is_mobile() === true || $this->_mobile->is_tablet() === true) ) {
				$has_checked_for_foobar = true;
				$has_foobar             = false;
...
//          }
		...
		function inline_dynamic_js() {
			...
			if (!apply_filters('show_foobar', true)) return;
			$include_js = false;
