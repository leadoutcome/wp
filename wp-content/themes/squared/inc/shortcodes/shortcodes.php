...
function thrive_shortcode_page_section( $attr, $content ) {
	...
	if ( $attr['template'] == 0 ) {
		...
		switch ( $attr['template'] ) {
			case 1:
				$output = "<div class='clear'></div><div class='" . $out_class . " pattern1' style='" . $shadow_string . "'><div class='in  darkSec'><div class='cck clearfix'><p class='lightSec'>" . do_shortcode( $content ) . "</p></div></div></div>";
				break;
			case 2:
				$output = "<div class='clear'></div><div class='" . $out_class . " pattern2' style='" . $shadow_string . "'><div class='in  lightSec'><div class='cck clearfix'><p class='lightSec'>" . do_shortcode( $content ) . "</p></div></div></div>";
				break;
			case 3:
				$output = "<div class='clear'></div><div class='" . $out_class . " pattern3' style='" . $shadow_string . "'><div class='in  darkSec'><div class='cck clearfix'><p class='lightSec'>" . do_shortcode( $content ) . "</p></div></div></div>";
				break;
		}
	}
...
function thrive_shortcode_text_block( $attr, $content ) {
	...
	if ( empty( $attr['headline'] ) ) {
        ...
        <div class='thrivecb " . $attr['color'] . "'><div class='" . $colour_scheme . "'>" . do_shortcode( $content ) . "</div></div>";
	} else {
		$output = "<div class='clear'></div><div class='thrivecb " . $attr['color'] . "'><div class='" . $colour_scheme . "'><h3>" . $attr['headline'] . "</h3>" . do_shortcode( $content ) . "</div></div>";
	}
...
function thrive_shortcode_pullquote( $attr, $content ) {
	...
	$output = "<blockquote class='" . $container_class . "'><span class='quo left'>&ldquo;</span><div class='left'><p>" . $content . "<p>" . $cite_html . "</div><div class='clear'></div></blockquote>";
...
function thrive_shortcode_posts_gallery( $attr, $content ) {
	...
	foreach ( $posts as $p ) {
		...
		$output .= "<div class='sht'><h5>" . get_the_title( $p->ID ) . "</h5></div>";
		$output .= "</a>";
		$output .= "</div>";
	}
	$output .= "</div>";
...
function thrive_shortcode_testimonal( $attr, $content ) {
	...
	$output      = "<div class='clear'></div><div class='" . $table_class . "'><div class='tst'><p> " . $content . " </p></div>";
	$output .= "<div class='bts clearfix'>";
	if ( ! empty( $attr['image'] ) ) {
		$output .= "<div class='left'><div class='rta' style=\"background-image: url('" . $attr['image'] . "')\"></div></div>";
	} else {
		$output .= "";
	}
	$output .= "<div class='left'><p>" . $attr['name'] . "</p><span>" . $attr['company'] . "</span></div></div></div>";
...
function thrive_shortcode_follow_me( $attr, $content ) {
	...
	if ( ! empty( $attr['facebook'] ) ) {
		...
		$output .= "<a class='fb' href='javascript:void(0)'><span class='awe'>&#xf09a;</span></a></div>";
	}
	if ( ! empty( $attr['twitter'] ) ) {
		...
		$output .= "<a class='tw' href='javascript:void(0)'><span class='awe'>&#xf099;</span></a><script>jQuery(function() { ThriveApp.load_script('twitter'); });</script></div>";
	}
	if ( ! empty( $attr['gprofile'] ) ) {
		...
		$output .= "<a class='gg' href='javascript:void(0)'><span class='awe'>&#xf0d5;</span></a><script>jQuery(function() { ThriveApp.load_script('google'); });</script></div>";
	}
	if ( ! empty( $attr['gpage'] ) ) {
		$randId = rand( 9, 99999 );
		$output .= "<div class='ss left'>";
	...
	if ( ! empty( $attr['linkedin'] ) ) {
		...
		$output .= "<a class='lk' href='javascript:void(0)'><span class='awe'>&#xf0e1;</span></a><script>jQuery(function() { ThriveApp.load_script('linkedin'); });</script></div>";
	}
	...
	if ( ! empty( $attr['pinterest'] ) ) {
		...
		$output .= "<a class='pt' href='javascript:void(0)'><span class='awe'>&#xf0d2;</span></a><script>jQuery(function() { ThriveApp.load_script('pinterest'); });</script></div>";
	}
	if ( ! empty( $attr['rss'] ) ) {
		$randId = rand( 9, 99999 );
		$output .= "<div class='ss left'>";
	...
	if ( ! empty( $attr['youtube'] ) ) {
		...
		$output .= "<a class='yt' href='javascript:void(0)'><span class='awe'>&#xf167;</span></a><script>jQuery(function() { ThriveApp.load_script('youtube'); });</script></div>";
	}
...
function thrive_shortcode_custom_box( $attr, $content ) {
	...
	$output = "";
	if ( $attr['type'] == "image" ) {
		$output = "<div class='thrivecb image " . ( $attr['full_height'] == 1 ? "flh" : "" ) . "'  style='background-image: url(\"" . $attr['image'] . "\")' style='max-height: 953px; max-width: 1280px;'>";
		if ( $attr['full_height'] == 1 ) {
			$output .= "<img class='tt-dmy' src='" . $attr['image'] . "' alt=''/>";
		}
		$output .= "<div class='shn-i'><div class='shn" . ( $attr['style'] == "dark" ? "d" : "" ) . " shnt'>
                        <h3>" . $attr['title'] . "</h3>
...
function thrive_shortcode_fill_counter( $attr, $content ) {
	...
	$output = '<div class="fill-counter nsd ' . $attr['color'] . '">
                    <div class="fill-counter-inner" style="stroke-dasharray: ' . ( $attr['percentage'] * 6.35 ) . ' 635 ;">
                        <svg viewBox="0 0 202 202" class="fill-counter-circle" shape-rendering="optimizeSpeed">
