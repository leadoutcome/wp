	...
	function thrive_breadcrumbs() {
		...
		if ( is_category() || is_single() ) {
			if ( $post ) {
				...
				if ( $cat_name !== null ) {
					echo "<li typeof='v:Breadcrumb'><a rel='v:url' property='v:title' href='" . get_category_link( get_cat_ID( $cat_name ) ) . "'>" . $cat_name . "</a></li>";
				}
			...
			if ( $post->post_parent ) {
				...
				echo $output . "<li><a class='no-link' rel='v:url' property='v:title' href='" . get_permalink() . "'>";
				the_title();
				echo "</a></li>";
			} else {
...
function thrive_render_top_focus_area( $position = "top", $place = null ) {
	...
	if ( ! empty( $current_post ) && $current_post->post_type == "post" ) {
		if ( $post_focus ) {
	...
	<table class="form-table">
		...
		<tr>
			...
			<td>
				...
				<input type="checkbox" id="gauthor"
	...
	<table class="form-table">
		<tr>
			...
			<td>
				...
				<input id="show_facebook" value="fbk" type="checkbox"
					   name="show_social_profiles[]" <?php echo ( in_array( 'fbk', $show_social_profiles ) ) ? "checked" : ""; ?> />
				<label for="show_facebook"><?php _e( "Facebook", 'thrive' ); ?></label>
				<input id="show_twitter" value="twt" type="checkbox"
					   name="show_social_profiles[]" <?php echo ( in_array( 'twt', $show_social_profiles ) ) ? "checked" : ""; ?> />
				...
				<input id="show_googleplus" value="ggl" type="checkbox"
					   name="show_social_profiles[]" <?php echo ( in_array( 'ggl', $show_social_profiles ) ) ? "checked" : ""; ?> />
...
/**
 * Danut: to be removed if it's not needed any more
 * If there is any [text] between squared brackets it will not be displayed in P tags
 *
					...
					<div class="lrp" id="respond-container-<?php echo get_comment_ID(); ?>" style="display: none;">
						...
						<form action="<?php echo get_option( 'siteurl' ); ?>/wp-comments-post.php" method="post">
							...
							<input id="comment_parent" type="hidden" value="<?php echo get_comment_ID(); ?>"
								   name="comment_parent">
							<?php echo apply_filters( 'comments_recaptcha_html', '' ); ?>
							<div class="">
	...
	if ( $args['next_or_number'] == 'next_and_number' ) {
		...
		if ( $multipage ) {
			if ( ( intval( $page ) - 1 ) && $more ) {
				$prev = _wp_link_page( intval( $page ) - 1 ) . __( 'Previous', 'thrive' ) . '</a> ';
			}
			...
			if ( ( intval( $page ) + 1 ) <= $numpages && $more ) {
				$next = _wp_link_page( intval( $page ) + 1 ) . __( 'Next', 'thrive' ) . '</a> ';
			}
