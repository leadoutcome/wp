...
<table class="form-table postEdit">
	...
	<tr>
		<td><input class="button button-primary" type="button" id="thrive_shortcode_btn_add_toggle"
		           value="<?php _e( "Add toggle", 'thrive' ); ?>"/></td>
		<td>
...
<script type="text/javascript">
	...
	jQuery( document ).ready( function () {
		...
		jQuery( "#thrive_shortcode_btn_insert" ).click( function () {
			...
			for ( var i in titles ) {
				sc_text += '[thrive_toggles title="' + titles[i] + '" no="' + (
				           parseInt( i ) + 1
					) + '/' + contents.length + '"]' + contents[i] + '[/thrive_toggles]';
			}
