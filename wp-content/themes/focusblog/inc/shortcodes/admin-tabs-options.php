<div>
	...
	<table class="form-table">
		<tr>
			<th scope="row">
				<input class="button button-primary" type="button" id="thrive_shortcode_btn_add_new_tab"
				       value="<?php _e( "Add new tab", 'thrive' ); ?>"/>
			</th>
			<td>
				<input class="pure-button upload" type="button" id="thrive_shortcode_btn_insert"
	...
	jQuery( document ).ready( function () {
		...
		jQuery( '#thrive_shortcode_btn_insert' ).click( function () {
			...
			jQuery( ".txt-thrive-tab-headline" ).each( function ( index ) {
				...
				if ( headline_txt_val !== '' ) {
					if ( index === 0 ) {
						sc_text += '[thrive_tab headline="' + headline_txt_val + '" no="' + (
						           index + 1
							) + '/' + (
							           no_tabs
						           ) + '"]Enter your content here[/thrive_tab]';
