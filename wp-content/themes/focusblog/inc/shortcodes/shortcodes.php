	...
	foreach ( $menu_items as $item ) {
		if ( is_object( $item ) ) {
			if ( $attr['thumbnails'] == "on" ) {
				$featured_img_data = thrive_get_post_featured_image( $item->ID, "tt_post_icon" );
...
function thrive_shortcode_posts_gallery( $attr, $content ) {
	...
	foreach ( $posts as $p ) {
		...
		$output .= "</div>";
		$output .= "<a class='rmc' href='" . get_permalink( $p->ID ) . "'>" . $continueReadingLabel . "&raquo;</a>";
		$output .= "</div>";
	}
	$output .= "</div>";
...
function thrive_shortcode_follow_me( $attr, $content ) {
	...
	if ( ! empty( $attr['twitter'] ) ) {
		...
		$output .= "<a class='tw'><span></span></a><script>jQuery(function() { ThriveApp.load_script('twitter'); });</script></div>";
	}
	if ( ! empty( $attr['gprofile'] ) ) {
		...
		$output .= "<a class='gg'><span></span></a><script>jQuery(function() { ThriveApp.load_script('google'); });</script></div>";
	}
	if ( ! empty( $attr['gpage'] ) ) {
		$randId = rand( 9, 99999 );
		$output .= "<div class='ss left'>";
	...
	if ( ! empty( $attr['linkedin'] ) ) {
		...
		$output .= "<a class='lk'><span></span></a><script>jQuery(function() { ThriveApp.load_script('linkedin'); });</script></div>";
	}
	...
	if ( ! empty( $attr['pinterest'] ) ) {
		...
		$output .= "<a class='pt'><span></span></a><script>jQuery(function() { ThriveApp.load_script('pinterest'); });</script></div>";
	}
	if ( ! empty( $attr['rss'] ) ) {
		$randId = rand( 9, 99999 );
		$output .= "<div class='ss left'>";
	...
	if ( ! empty( $attr['youtube'] ) ) {
		...
		$output .= "<a class='yt'><span></span></a><script>jQuery(function() { ThriveApp.load_script('youtube'); });</script></div>";
	}
