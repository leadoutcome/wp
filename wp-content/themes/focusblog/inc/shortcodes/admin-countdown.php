...
<script type="text/javascript">
	...
	jQuery( document ).ready( function ( $ ) {
		...
		var $date_time_picker = $( '.dateTimePicker' );
		...
		jQuery( "#thrive_shortcode_btn_insert" ).click( function () {
			...
			if ( jQuery( '#thrive_shortcode_countdown_datetime' ).val().length <= 0 ) {
				return;
			}

			var date = $date_time_picker.datetimepicker( 'getDate' ),
				sc_options = {
			...
			tb_remove();
			var sc_text = '[thrive_countdown color="' + sc_options.color + '" year="' + sc_options.year + '" month="' + sc_options.month + '" day="' + sc_options.day + '" hour="' + sc_options.hour + '" min="' + sc_options.minute + '" fade="' + sc_options.fade + '" text="' + sc_options.text + '"]';
