...
class Thrive_Optin_Widget extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		?>
		<section class="optin_widget <?php echo $widget_container_class; ?>" id="<?php echo $args['widget_id']; ?>">
			<div class="oab">
				<h4 class="upp"><?php echo $instance['headline_text']; ?></h4>
				<?php if ( $instance['custom_image'] && $instance['custom_image'] != "" ): ?>
					<img src="<?php echo $instance['custom_image']; ?>" alt="">
				<?php endif; ?>
				<p class="optin_content"><?php echo $instance['body_text']; ?></p><br/>

				<form action="<?php echo $optinFormAction; ?>" method="<?php echo $optinFormMethod ?>" class="frm">
					...
					<input type="submit" class="optin_submit" value="<?php echo $instance['button_text']; ?>"/>
			...
			<select name='<?php echo esc_attr( $this->get_field_name( 'optin' ) ); ?>'>
				...
				<?php foreach ( $queryOptins->get_posts() as $p ): ?>
					<option value='<?php echo $p->ID ?>'
					        <?php if ( $optin == $p->ID ): ?>selected<?php endif; ?>><?php echo get_the_title( $p->ID ); ?></option>
				<?php endforeach; ?>
			</select></p>
