...
class Thrive_Custom_Phone extends WP_Widget {
	...
	public function __construct() {
		$widget_ops = array(
			'classname'   => 'widget_thrive_phone',
			'description' => __( 'Click-to-Call phone number with a call to action text line.', 'thrive' )
		);
	...
	function widget( $args, $instance ) {
		...
		<section class="widget phone-number" id="<?php echo $args['widget_id']; ?>">
			<div class="scn">
				<div class="awr">
					<div class="phone <?php echo $instance['color'] ?>" id="<?php echo $args['widget_id']; ?>">
	...
	function form( $instance ) {
		...
		<p><label
			...
			<input class="widefat" type="text" id="<?php echo esc_attr( $this->get_field_id( 'custom_phone' ) ); ?>"
			       name="<?php echo esc_attr( $this->get_field_name( 'custom_phone' ) ); ?>"
			       value="<?php echo esc_attr( $custom_phone ); ?>"/>
		</p>

		<p><label
				for="<?php echo esc_attr( $this->get_field_id( 'color' ) ); ?>"><?php _e( 'Mobile button color', 'thrive' ); ?></label>
