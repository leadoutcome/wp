<?php
/**
 * Flickr widget for childgenesis
 * 
 * @package    Genesis Child Theme
 * @subpackage Widgets
 * @author     childgenesis Developer
 * @version    1.0
 * @link       http://wwww.childgenesis.com/
 */
...
class GA_Flickr_Widget extends WP_Widget {
	...
	function __construct() {
		$widget_ops = array( 'classname' => 'ga-flickr', 'description' => __( 'Displays Flickr photos stream on sidebar', 'childgenesis' ) );
		$this->WP_Widget( 'ga-flickr', __( 'ChildGenesis : Flickr', 'childgenesis' ), $widget_ops );
	}
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'title' ) ?>"><?php esc_html_e( 'Title', 'childgenesis' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'flickrid' ) ?>"><?php esc_html_e( 'Flickr ID', 'childgenesis' ); ?> (<a href="http://www.idgettr.com" target="_blank">idGettr</a>)</label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'numberphotos' ) ?>"><?php esc_html_e( 'Number of Photos', 'childgenesis' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'streamtype' ) ?>"><?php esc_html_e( 'Type:', 'childgenesis' ); ?></label>
			<select class="widefat" name="<?php echo $this->get_field_name( 'streamtype' ) ?>" id="<?php echo $this->get_field_id( 'streamtype' ) ?>">
				<option value="user"<?php selected( 'user', $streamtype ); ?>><?php esc_html_e( 'User', 'childgenesis' ) ?></option>
				<option value="group"<?php selected( 'group', $streamtype ); ?>><?php esc_html_e( 'Group', 'childgenesis' ) ?></option>
			</select>
		</p>
		<p>
			<label for="<?php echo $this->get_field_id( 'streamsorting' ) ?>"><?php esc_html_e( 'Sorting:', 'childgenesis' ); ?></label>
			<select class="widefat" name="<?php echo $this->get_field_name( 'streamsorting' ) ?>" id="<?php echo $this->get_field_id( 'streamsorting' ) ?>">
				<option value="latest"<?php selected( 'latest', $streamsorting ); ?>><?php esc_html_e( 'Latest', 'childgenesis' ) ?></option>
				<option value="random"<?php selected( 'random', $streamsorting ); ?>><?php esc_html_e( 'Random', 'childgenesis' ) ?></option>
			</select>
		</p>
		<p>
			<label for="<?php echo $this->get_field_id( 'photosize' ) ?>"><?php esc_html_e( 'Size:', 'childgenesis' ); ?></label>
			<select class="widefat" name="<?php echo $this->get_field_name( 'photosize' ) ?>" id="<?php echo $this->get_field_id( 'photosize' ) ?>">
				<option value="s"<?php selected( 's', $photosize ); ?>><?php esc_html_e( 'Small', 'childgenesis' ) ?></option>
				<option value="l"<?php selected( 'l', $photosize ); ?>><?php esc_html_e( 'Medium', 'childgenesis' ) ?></option>
				<option value="m"<?php selected( 'm', $photosize ); ?>><?php esc_html_e( 'Large', 'childgenesis' ) ?></option>
			</select>
		</p>
