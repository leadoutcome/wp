...
$section_class     = $options['sidebar_alignement'] == "left" ? "right" : ( $options['sidebar_alignement'] == "right" ? "left" : "" );
...
<?php get_template_part( "appr/header" ); ?>
	<div class="wrp cnt">
		...
		<div class="<?php echo $section_class === 'fullWidth' ? '' : 'bSeCont' ?>">
			<?php endif; ?>
			<section class="bSe <?php echo $section_class; ?>">
						...
						if ( isset( $options['bottom_previous_next'] ) && $options['bottom_previous_next'] == 1 && ( $next_lesson_link != false || $prev_lesson_link != false ) ):
							...
							<div class="awr ctr pgn">
								<?php if ( $prev_lesson_link ): ?>
									<a class="page-numbers nxt" href='<?php echo $prev_lesson_link; ?>'>&laquo;&nbsp;<?php _e( "Previous lesson", 'thrive' ); ?> </a>
								<?php endif; ?>
								<?php if ( $next_lesson_link ): ?>
									<a class="page-numbers prv" href='<?php echo $next_lesson_link; ?>'><?php _e( "Next lesson", 'thrive' ) ?>&nbsp;&raquo;</a>
								<?php endif; ?>
							</div>
						<?php endif; ?>
				...
				<?php else: ?>
					<div class="no_content_msg">
						...
						<p class="ncm_comment"><?php _e( "YOU CAN RETURN", 'thrive' ); ?> <a
								href="<?php echo home_url( '/' ); ?>"><?php _e( "HOME", 'thrive' ) ?></a> <?php _e( "OR SEARCH FOR THE PAGE YOU WERE LOOKING FOR.", 'thrive' ) ?></p>
