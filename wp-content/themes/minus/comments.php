...
<?php if ( $enable_fb_comments != "only_fb" ): ?>
	<article id="comments">
		<?php if ( comments_open() && ! post_password_required() ) : ?>
			<div class="ctb">
				<h3><span class="txt_thrive_link_to_comments"><?php _e( "Click Here to Leave a Comment Below", 'thrive' ); ?></span></h3>
				<div class="clear"></div>
			</div>
		<?php endif; ?>
		...
		<?php if ( comments_open() && ! post_password_required() ) : ?>
			...
			<div class="lrp" id="thrive_container_form_add_comment" <?php if ( $lazy_load_comments == 1 ): ?>style="display:none;"<?php endif; ?>>
				<h6 class="left"><?php _e( "Leave a Reply:", 'thrive' ); ?></h6>
				<div class="clear"></div>
				<form action="<?php echo get_option( 'siteurl' ); ?>/wp-comments-post.php" method="post" id="commentform">
					<textarea class="textarea" name="comment" id="comment" placeholder="<?php _e( "Share your thoughts...", 'thrive' ); ?>"></textarea>
					...
					?>
					<input type="submit" value="<?php _e( "SUBMIT", 'thrive' ); ?>" class="right"/>
					<div class="clear"></div>
