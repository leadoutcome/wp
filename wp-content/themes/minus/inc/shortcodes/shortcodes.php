...
function thrive_shortcode_custom_menu( $attr, $content ) {
	...
	foreach ( $menu_items as $item ) {
		...
		if ( $attr['thumbnails'] == "on" ) {
			...
			$output .= "<a href='" . $item->url . "'>" . get_the_title( $item->object_id ) . "</a>";
...
function thrive_shortcode_page_section( $attr, $content ) {
	...
	if ( $attr['template'] == 0 ) {
		...
		switch ( $attr['template'] ) {
			case 1:
				$output = "<div class='clear'></div><div class='" . $out_class . " pattern1' style='" . $shadow_string . "'><div class='in  lightSec'><div class='cck clearfix'><p class='lightSec'>" . do_shortcode( $content ) . "</p></div></div></div>";
				break;
			case 2:
				$output = "<div class='clear'></div><div class='" . $out_class . " pattern2' style='" . $shadow_string . "'><div class='in  darkSec'><div class='cck clearfix'><p class='lightSec'>" . do_shortcode( $content ) . "</p></div></div></div>";
				break;
			case 3:
				$output = "<div class='clear'></div><div class='" . $out_class . " pattern3' style='" . $shadow_string . "'><div class='in  lightSec'><div class='cck clearfix'><p class='lightSec'>" . do_shortcode( $content ) . "</p></div></div></div>";
				break;
		}
	}
...
function thrive_shortcode_text_block( $attr, $content ) {
	...
	if ( empty( $attr['headline'] ) ) {
		...
		$output = "<div class='clear'></div><div class='thrivecb " . $attr['color'] . "'><div class='" . $colour_scheme . "'><h2>" . $attr['headline'] . "</h2><p>" . do_shortcode( $content ) . "</p></div></div>";
	}
...
function thrive_shortcode_posts_gallery( $attr, $content ) {
	...
	foreach ( $posts as $p ) {
		...
		if ( $featured_img ) {
			...
			$output .= "<div class='rimc' style='background-image: url(\"" . get_template_directory_uri() . "/images/default.png\");'></div>";
		}
		$output .= "<h4>" . get_the_title( $p->ID ) . "</h4>";
		$output .= "</a>";
		$output .= "</div>";
	}
	$output .= "</div>";
	...
	foreach ( $posts as $p ) {
		if ( $attr['thumbnails'] == "on" ) {
			...
			if ( $featured_img ) {
				...
				$output .= "<a href='" . get_permalink( $p->ID ) . "' style='background-image: url(\"" . get_template_directory_uri() . "/images/default.png\")'></a></div>";
			}
			$output .= "<div class='left txt'>";
			$output .= "<a href='" . get_permalink( $p->ID ) . "'>" . get_the_title( $p->ID ) . "</a>";
...
function thrive_shortcode_testimonal( $attr, $content ) {
	...
	$output = "<table class='sctm'><tr class='tst'><td colspan='3'> " . $content . " </td></tr><tr class='lts'><td class='ltso'></td><td class='ltst'><span></span></td><td></td></tr>";
	$output .= "<tr class='bts'>";
	if ( ! empty( $attr['image'] ) ) {
...
function thrive_shortcode_follow_me( $attr, $content ) {
	...
	if ( ! empty( $attr['facebook'] ) ) {
		...
		$output .= "<a class='fb' href='#'><span class='awe'>&#xf09a;</span></a></div>";
	}
	if ( ! empty( $attr['twitter'] ) ) {
		$randId = rand( 9, 99999 );
		$output .= "<div class='ss left'>";
		$output .= "<div class='bubb'><div class='bubble' id='bubble-" . $randId . "'>";
		$output .= "<a href='" . _thrive_get_social_link( $attr['twitter'], 'twitter' ) . "' class='twitter-follow-button' data-show-count='false'>Follow @" . $attr['twitter'] . "</a></div></div>";
		$output .= "<a class='tw' href='#'><span class='awe'>&#xf099;</span></a><script>jQuery(function() { ThriveApp.load_script('twitter'); });</script></div>";
	}
	if ( ! empty( $attr['gprofile'] ) ) {
		...
		$output .= "<a class='gg' href='#'><span class='awe'>&#xf0d5;</span></a><script>jQuery(function() { ThriveApp.load_script('google'); });</script></div>";
	}
	if ( ! empty( $attr['gpage'] ) ) {
		$randId = rand( 9, 99999 );
		$output .= "<div class='ss left'>";
	...
	if ( ! empty( $attr['linkedin'] ) ) {
		...
		$output .= "<a class='lk' href='#'><span class='awe'>&#xf0e1;</span></a><script>jQuery(function() { ThriveApp.load_script('linkedin'); });</script></div>";
	}
	if ( ! empty( $attr['dribble'] ) ) {
		$randId = rand( 9, 99999 );
		$output .= "<div class='ss left'>";
		$output .= "<a class='dr' href='" . _thrive_get_social_link( $attr['dribble'], 'dribble' ) . "'><span class='awe'>&#xf17d;</span></a></div>";
	}
	if ( ! empty( $attr['pinterest'] ) ) {
		...
		$output .= "<a class='pt' href='#'><span class='awe'>&#xf0d2;</span></a><script>jQuery(function() { ThriveApp.load_script('pinterest'); });</script></div>";
	}
	if ( ! empty( $attr['rss'] ) ) {
		$randId = rand( 9, 99999 );
		$output .= "<div class='ss left'>";
		$output .= "<a class='rs' href='" . $attr['rss'] . "'><span class='awe'>&#xf09e;</span></a></div>";
	}
	if ( ! empty( $attr['youtube'] ) ) {
		...
		$output .= "<a class='yt' href='#'><span class='awe'>&#xf16a;</span></a><script>jQuery(document).ready(function() { ThriveApp.load_script('youtube'); });</script></div>";
	}
	if ( ! empty( $attr['instagram'] ) ) {
		...
		$output .= "<a class='is'><span class='awe'>&#xf16d;</span></a></div>";
	}
	if ( ! empty( $attr['xing'] ) ) {
		$randId = rand( 9, 99999 );
		$output .= "<div class='ss left'>";
		$output .= "<a class='xi' href='" . $attr['xing'] . "'><span class='awe'>&#xf168;</span></a></div>";
	}
...
function thrive_shortcode_borderless( $attr, $content ) {
	...
	switch ( $attr['type'] ) {
		...
		case 'youtube':
			...
			break;
		case 'vimeo':
			$output = "<div class='" . $container_class . " " . $hasControls . " brve'>" . _thrive_get_vimeo_embed_code( $content ) . "</div>";
			break;
		case 'custom_code':
...
function thrive_shortcode_content_container( $attr, $content ) {
	...
	$style_str   = " style='max-width: " . $attr['max_width'] . "px;'";
...
function thrive_shortcode_custom_box( $attr, $content ) {
	...
	if ( $attr['type'] == "image" ) {
		...
		if ( $attr['full_height'] == 1 ) {
			$output .= "<img class='tt-dmy' src='" . $attr['image'] . "' alt=''/>
                            <div class='shn-i'>
                            <div class='shn" . ( $attr['style'] == "dark" ? "d" : "" ) . " '>
                                <h4>" . $attr['title'] . "</h4>
                                    <p>" . do_shortcode( $content ) . "</p>
                            </div></div>";
		} else {
			$output .= "<div class='shn-i'><div class='shn" . ( $attr['style'] == "dark" ? "d" : "" ) . "'>
                            <h2>" . $attr['title'] . "</h2>
                                <p>" . do_shortcode( $content ) . "</p>
                        </div></div>";
		}
...
function thrive_shortcode_fill_counter( $attr, $content ) {
	...
	$output = '<div class="fill-counter nsd ' . $attr['color'] . '">
                    <div class="fill-counter-inner" style="stroke-dasharray: ' . ( $attr['percentage'] * 7.73 ) . ' 773 ;">
                        <svg viewBox="0 0 246 246" class="fill-counter-circle" shape-rendering="optimizeSpeed">
                            <circle r="123" cx="123" cy="123"></circle>
                        </svg>
...
function thrive_shortcode_headline_focus( $attr, $content ) {
	...
	$output = '<div class="fhd ' . $attr['orientation'][0] . 'fhd">
                        ' . ( ( $attr['orientation'] != 'left' ) ? '<div class="fhdl"></div>' : '' ) . '
                    <h3 class="fhdt">
                        ' . $attr['title'] . '
                    </h3>
                    ' . ( ( $attr['orientation'] != 'right' ) ? '<div class="fhdl"></div>' : '' ) . '
                </div>';
