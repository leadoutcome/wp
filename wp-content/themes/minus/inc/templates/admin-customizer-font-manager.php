...
$imported_fonts_link = Tve_Dash_Font_Import_Manager::getCssFile();
	...
	<div>
		...
		<select id="ttfm_fonts">
			...
			<?php foreach ( $prefered_fonts as $name => $f ): ?>
				<option
					data-url='<?php echo json_encode( $f['files'] ); ?>' <?php if ( isset( $f['font_name'] ) && $f['family'] == $f['font_name'] ) {
					echo 'selected';
				} ?>
		...
		jQuery( '.save-modal' ).click( function () {
			...
			if ( font_bold === undefined ) {
				jQuery.each( font_variants, function ( key, value ) {
					...
					if ( font_bold === undefined && ! isNaN( _value ) && _value > 400 && (
						isNaN( font_style ) || (
						! isNaN( font_style ) && _value > font_style
						)
						) ) {
						font_bold = value;
					}
			...
			var font = "//fonts.googleapis.com/css?family=" + font_name.replace( ' ', '+' ) + (
					font_style !== undefined ? ":" + font_style : ""
				) + (
				           font_italic !== undefined ? font_italic : ""
			           ) + (
				           font_bold !== undefined ? "," + font_bold : ""
			           ) + (
				           font_set !== undefined ? "&subset=" + font_set : ""
			           );
