...
function thrive_appr_get_apprentice_single_templates( $single_template ) {
	...
	if ( $post->post_type == TT_APPR_POST_TYPE_LESSON || $post->post_type == TT_APPR_POST_TYPE_PAGE ) {
		...
		$single_template = "/appr/appr-lesson.php";

		if ( $post->post_type == TT_APPR_POST_TYPE_PAGE ) {
			$single_template = "/appr/appr-page.php";
		}
		...
		switch ( $post_template ) {
			case "Full Width" :
				$single_template = '/appr/appr-full-width.php';
				break;
			case "Narrow" :
				$single_template = '/appr/appr-narrow.php';
				break;
			case "Landing Page" :
				$single_template = '/appr/appr-landing-page.php';
				break;
		}

		if ( ! is_file( get_stylesheet_directory() . $single_template ) ) {
			$single_template = TEMPLATEPATH . $single_template;
		} else {
			$single_template = get_stylesheet_directory() . $single_template;
		}
