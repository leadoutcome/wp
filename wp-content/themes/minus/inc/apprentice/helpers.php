...
function _thrive_get_next_prev_lesson_link( $lessonId, $next = true ) {
	...
	$index    = 0;
	$newOrder = array();
	while ( $index < count( $lessonsOrder ) ) {
		foreach ( $lessonsOrder as $catId => $lessonsIds ) {
			if ( $lessonsIds['order'] == $index ) {
				$newOrder[] = $lessonsIds;
			}
		}
		$index ++;
	...
	$all_lessons = array();
	foreach ( $newOrder  as $catId => $lessonsIds ) {
...
function _thrive_get_ordered_cat_array_by_parent( $parent_id = 0 ) {
	...
	$cats = get_terms( "apprentice", 'hide_empty=0&posts_per_page=-1&parent=' . $parent_id );
...
function _thrive_appr_get_lessons_root_url() {
	...
	$homepage_url = get_permalink( $root_page_id );
