...
if ( ! function_exists( 'thrive_breadcrumbs' ) ) :
	function thrive_breadcrumbs() {
		...
		if ( get_option( 'show_on_front' ) == 'page' ) {
			...
			if ( ! is_page() && ! empty( $posts_page_id ) ) {
				if ( ! is_home() ) {
					...
					echo "<li typeof='v:Breadcrumb'><a rel='v:url' property='v:title' class='home' href='" . get_option( 'home' ) . "'> " . __( "Blog", 'thrive' ) . $arrowImg . "</a></li>";
				}
			}
		} else {
		...
		if ( is_category() || is_single() ) {
			...
			echo "<li typeof='v:Breadcrumb'>" . __( "Search results", 'thrive' ) . ": </li>";
		} elseif ( is_archive() ) {
			echo "<li typeof='v:Breadcrumb'>" . __( "Archive", 'thrive' ) . ": </li>";
		}
...
function thrive_render_top_focus_area( $position = "top", $place = null, $return_content = false ) {
	global $post;
	if ( empty( $post ) ) {
		return;
	}
	...
	if ( $return_content ) {
		return ( isset( $current_attrs['_thrive_meta_focus_subheading_text'][0] ) ) ? $current_attrs['_thrive_meta_focus_subheading_text'][0] : '';
	}
...
function thrive_more_link( $more_link, $more_link_text ) {
	...
	$read_more_class  = ( $options['other_read_more_type'] == "button" ) ? "btn read small" : "readmore_link";
...
function thrive_add_tcb_page_sections_colors( $colours ) {
	...
	$colours['pattern1'] = array( 'color' => '#2C2C2C', 'shadow' => '', 'image' => '', 'pattern' => '', 'textstyle' => 'light' );
	$colours['pattern2'] = array( 'color' => '#DFDFDF', 'shadow' => '', 'image' => '', 'pattern' => '', 'textstyle' => 'dark' );
	$colours['pattern3'] = array( 'color' => '#537EA2', 'shadow' => '', 'image' => '', 'pattern' => '', 'textstyle' => 'light' );
			...
			default :
				...
				<div class="cmb" id="comment-<?php echo get_comment_ID(); ?>">
					<div class="<?php echo $comment_container_class; ?>">
						<div class="left">
							<?php echo get_avatar( get_comment_author_email(), 108 ); ?>
						</div>
						...
						<div class="right">
							...
							<?php if ( comments_open() && ! post_password_required() && '0' != $comment->comment_approved ) : ?>
								<a class="rpl reply" href="#" id="link-reply-<?php echo get_comment_ID(); ?>"
					...
					<div class="lrp" id="respond-container-<?php echo get_comment_ID(); ?>" style="display: none;">
						...
						<form action="<?php echo get_option( 'siteurl' ); ?>/wp-comments-post.php" method="post">
                                <textarea class="textarea" name="comment" id="comment"
                                          placeholder="<?php _e( "Share your thoughts...", 'thrive' ); ?>"></textarea>
							...
							<input type="submit" value="<?php _e( "Submit comment", 'thrive' ); ?>" class="right">
...
function thrive_save_post_font( $post_id ) {
	...
	if ( empty( $post_content ) ) {
		$post_content = get_post_meta( $post_id, '_thrive_meta_focus_subheading_text', true );
	}
...
function thrive_enqueue_head_fonts() {
	...
	$focus_areas = thrive_render_top_focus_area( 'top', null, true ) . thrive_render_top_focus_area( 'bottom', null, true );

	preg_match_all( "/thrive_custom_font id='\d+'/", $focus_areas, $font_ids );
	...
	foreach ( $font_ids[0] as $font_id ) {
		...
		$font  = thrive_get_font_options( $id );
		if ( isset( $font ) ) {
			$font_url = "//fonts.googleapis.com/css?family=" . str_replace( " ", "+", $font->font_name ) . ( $font->font_style != 0 ? ":" . $font->font_style : "" ) . ( $font->font_bold != 0 ? "," . $font->font_bold : "" ) . ( $font->font_italic != 0 ? $font->font_italic : "" ) . ( $font->font_character_set != 0 ? "&subset=" . $font->font_character_set : "" );
			wp_enqueue_style( 'tcf_' . md5( $font_url ), $font_url );
		}
	}
}
...
function thrive_theme_comment_nav() {
	...
	if ( get_comment_pages_count() > 1 && get_option( 'page_comments' ) ) {
		echo "<div class='com-pgn ctr pgn'>";
