...
<table class="form-table postEdit">
	...
	<tr>
		<td><input class="button button-primary" type="button" id="thrive_shortcode_btn_add_accordion"
		           value="<?php _e( "Add accordion", 'thrive' ); ?>"/></td>
		<td>
...
<script type="text/javascript">
	...
	jQuery( document ).ready( function () {
		...
		jQuery( "#thrive_shortcode_btn_insert" ).click( function () {
			var default_accordion = (
				                        jQuery( 'input:radio[name=default-accordion]:checked' ).parent().parent().index()
			                        ) / 3 - 1;
			...
			for ( var i in titles ) {
				sc_text += '[thrive_accordion title="' + titles[i] + '" no="' + (
				           parseInt( i ) + 1
					) + '/' + contents.length + '" default="' + (
					           default_accordion == i ? 'yes' : 'no'
				           ) + '"]' + contents[i] + '[/thrive_accordion]';
			}
