...
function thrive_shortcode_custom_menu( $attr, $content ) {
	...
	foreach ( $menu_items as $item ) {
		...
		if ( $attr['thumbnails'] == "on" ) {
			$featured_img      = _thrive_get_featured_image( $item->object_id, "medium" );
			...
			if ( $featured_img ) {
				...
				$output .= "<a href='" . $item->url . "' style='background-image: url(\"" . get_template_directory_uri() . "/images/default_post_list.png\")'></a></div>";
			}
			$output .= "<div class='left txt'>";
			$output .= "<a href='" . $item->url . "'>" . get_the_title( $item->object_id ) . "</a><br/>";
...
function thrive_shortcode_posts_gallery( $attr, $content ) {
	...
	if ( ! empty( $attr['title'] ) ) {
		$output .= "<table class='bth'><tr><td class='hsp'><span></span></td><td class='ahsp'><h3>" . $attr['title'] . "</h3></td><td class='hsp'><span></span></td></tr></table>";
	}

	$readMoreText         = thrive_get_theme_options( "other_read_more_text" );
	$continueReadingLabel = ( ! empty( $readMoreText ) ) ? $readMoreText : __( "Continue reading", 'thrive' );
	...
	foreach ( $posts as $p ) {
		...
		if ( $featured_img ) {
			...
			$output .= "<div class='rimc def' style='background-image: url(\"" . get_template_directory_uri() . "/images/default_post_gallery.png\");'></div>";
		}
		$output .= "<h4>" . get_the_title( $p->ID ) . "</h4>";
		$output .= "</a>";
		$output .= "<div class='bgin text-truncate'><p>" . $post_content_excerpt . "</p>";
		$output .= "</div>";
		$output .= "<a class='mre' href='" . get_permalink( $p->ID ) . "'><span>" . $continueReadingLabel . "</span><span class='awe'>&#xe608;</span><span class='clear'></span></a>";
		$output .= "";
		$output .= "</div>";
	}
	$output .= "</div>";
...
function thrive_shortcode_posts_list( $attr, $content ) {
	...
	foreach ( $posts as $p ) {
		if ( $attr['thumbnails'] == "on" ) {
			...
			$output .= "<div class='left txt'>";
			$output .= "<a href='" . get_permalink( $p->ID ) . "'>" . get_the_title( $p->ID ) . "</a><br/>";
...
function thrive_shortcode_follow_me( $attr, $content ) {
	...
	if ( ! empty( $attr['facebook'] ) ) {
		...
		$output .= "<a class='fb left' href='#'><span>&#xe60b;</span></a></div>";
	}
	if ( ! empty( $attr['twitter'] ) ) {
		...
		$output .= "<a class='tw left' href='#'><span>&#xe60a;</span></a><script>jQuery(function() { ThriveApp.load_script('twitter'); });</script></div>";
	}
	if ( ! empty( $attr['gprofile'] ) ) {
		...
		$output .= "<a class='gg left' href='#'><span>&#xe60c;</span></a><script>jQuery(function() { ThriveApp.load_script('google'); });</script></div>";
	}
	if ( ! empty( $attr['gpage'] ) ) {
		$randId = rand( 9, 99999 );
		$output .= "<div class='ss left'>";
	...
	if ( ! empty( $attr['linkedin'] ) ) {
		...
		$output .= "<a class='lk left' href='#'><span>&#xe60e;</span></a><script>jQuery(function() { ThriveApp.load_script('linkedin'); });</script></div>";
	}
	if ( ! empty( $attr['dribble'] ) ) {
		$randId = rand( 9, 99999 );
		$output .= "<div class='ss left'>";
		$output .= "<a class='dr left' href='" . $attr['dribble'] . "'><span>&#xe60f;</span></a></div>";
	}
	if ( ! empty( $attr['pinterest'] ) ) {
		...
		$output .= "<a class='pt left' href='#'><span>&#xe60d;</span></a><script>jQuery(function() { ThriveApp.load_script('pinterest'); });</script></div>";
	}
	if ( ! empty( $attr['rss'] ) ) {
		$randId = rand( 9, 99999 );
		$output .= "<div class='ss left'>";
		$output .= "<a class='rs left' href='" . $attr['rss'] . "'><span>&#xe603;</span></a></div>";
	}
	if ( ! empty( $attr['youtube'] ) ) {
		...
		$output .= "<a class='yt left' href='#'><span>&#xf167;</span></a><script>jQuery(document).ready(function() { ThriveApp.load_script('youtube'); });</script></div>";
	}
...
function thrive_shortcode_fill_counter( $attr, $content ) {
	...
	$output = '<div class="fill-counter nsd ' . $attr['color'] . '">
                    <div class="fill-counter-inner" style="stroke-dasharray: ' . ( $attr['percentage'] * 5.66 ) . ' 566 ;">
                        <svg viewBox="0 0 180 180" class="fill-counter-circle" shape-rendering="optimizeSpeed">
