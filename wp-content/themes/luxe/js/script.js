/*
 * 
 * Namespace for all the scripts implemented in the fronten
 */
...
//        jQuery('h4 span', this).toggleClass('oFaq');
...
ThriveApp.shortcodeTabsResize = function () {
	jQuery( ".bSe .scT ul.scT-tab li" ).on( 'click', function ( e ) {
		var $li = jQuery( this ),
...
ThriveApp.menuResponsive = function () {
	jQuery( 'header nav' ).show( 0 );
	if ( ThriveApp.winWidth <= 772 ) {
		jQuery( 'header nav ul li' ).each( function () {
	...
	} else if ( ThriveApp.winWidth >= 773 ) {
...
jQuery( window ).scroll( function () {
	...
	if ( hasTitle == 1 ) {
		...
		condition = position + 80 > containerTopPosition;
	}
	if ( condition ) {
