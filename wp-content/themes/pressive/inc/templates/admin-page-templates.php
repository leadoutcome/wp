...
<div class="thrive-page-settings">
	...
	<div id="thrive_conainter_page_tpl_privacy">
		...
		<form method="post" action="options.php" class="page-section-options"
		      id="thrive_form_page_tpl_privacy" <?php echo $privacy_container_style; ?>>
			<br/><br/>
			<div>
				<?php
				settings_fields( 'thrive_options' );
				do_settings_sections( 'theme_page_templates_options' );
				submit_button( __( 'Update Information', 'thrive' ) );
				?>
			</div>
			<br/><br/>
		</form>
	...
	<?php if ( $test_posts_generated ): ?>
		<?php _e( "Test posts generated successfully.", 'thrive' ); ?>
	<?php endif; ?>
	<form method="post" action="">
		<input type="hidden" name="thrive_generate_test_posts" value="1"/>
		<input class="button button-primary" type="submit" value="<?php _e( "Generate Test Posts", 'thrive' ); ?>"/>
	</form>
	<br/><br/>
	<form action="<?php echo admin_url( "admin.php?page=thrive_admin_page_templates&noheader=true" ); ?>" id=""
	      method="POST">
		<h3><?php _e( "Page templates", 'thrive' ); ?></h3>
		<br/>
		<table class="page-template-table">
			...
			<tr>
				<td colspan="2">
					<input type="checkbox" id="chk_page_tpl_homepage3" name="page_tpl_homepage3"/>
					<label class='ptf' for="chk_page_tpl_homepage3">Homepage 3</label>
				</td>
			</tr>
			...
			<tr>
				<td colspan="2">
					...
					<label class='ptf' for="chk_all_tpls"><b><?php _e( "Check all", 'thrive' ); ?></b></label>
				</td>
			</tr>
			...
			<tr <?php if ( ! defined( "TVE_VERSION" ) ): ?>style="display:none;"<?php endif; ?>>
				<td colspan="2">
					...
					<label class='ptf'
					       for="editable_with_tcb"><b><?php _e( "Make these pages editable with Thrive Content Builder", 'thrive' ); ?></b></label>
				</td>
			</tr>
		...
		<hr/>
		<br/>
		<input class="button button-primary" type="submit" value="<?php _e( 'Generate Selected Pages', 'thrive' ); ?>"/>
	</form>
</div>
...
<script type="text/javascript">
	jQuery( document ).ready( function () {
		...
		jQuery( "#thrive_chk_enable_privacy" ).click( function () {
			jQuery( "#thrive_form_page_tpl_privacy" ).toggle();
