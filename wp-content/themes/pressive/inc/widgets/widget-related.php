...
class Thrive_Related_Widget extends WP_Widget {
	...
	public function __construct() {
		$widget_ops = array(
			'classname'   => 'widget_thrive_related',
			'description' => __( 'Styled list of recent, popular or related posts.', 'thrive' )
		);
	...
	function widget( $args, $instance ) {
		...
		<section class="rw widget_thrive_related" id="<?php echo $args['widget_id']; ?>">
			<div class="scn">
				<div class="awr">
					<div class="twr">
								...
								<div class="left tim">
									<?php if ( isset( $instance['display_thumbnails'] ) && $instance['display_thumbnails'] == 1 ): ?>
										<?php if ( isset( $post['image'] ) && $post['image'] ): ?>
											<div class="wti"
											     style="background-image: url('<?php echo $post['image']; ?>')"></div>
										<?php else: ?>
											<div class="wti"
											     style="background-image: url('<?php echo get_template_directory_uri(); ?>/images/tabs_default.png')"></div>
										<?php endif; ?>
									<?php endif; ?>
								</div>
	...
	function form( $instance ) {
		...
		<p><label
				for="<?php echo esc_attr( $this->get_field_id( 'list_type' ) ); ?>"><?php _e( 'List type', 'thrive' ); ?></label>
			<select id="<?php echo esc_attr( $this->get_field_id( 'list_type' ) ); ?>"
			        name="<?php echo esc_attr( $this->get_field_name( 'list_type' ) ); ?>">
				<option value="recent"
				        <?php if ( $list_type == "recent" ): ?>selected<?php endif; ?>><?php _e( "Recent posts", 'thrive' ); ?></option>
				<option value="popular"
				        <?php if ( $list_type == "popular" ): ?>selected<?php endif; ?>><?php _e( "Popular posts", 'thrive' ); ?></option>
				<option value="related"
				        <?php if ( $list_type == "related" ): ?>selected<?php endif; ?>><?php _e( "Related Posts", 'thrive' ); ?></option>
			</select>
		</p>

		<p><label
				for="<?php echo esc_attr( $this->get_field_id( 'no_posts' ) ); ?>"><?php _e( 'Number of posts:', 'thrive' ); ?></label>
			<input class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'no_posts' ) ); ?>" type="text"
			       value="<?php echo $no_posts; ?>"
			       name="<?php echo esc_attr( $this->get_field_name( 'no_posts' ) ); ?>"/></p>
	...
	function _get_featured_image( $postId ) {
		...
		$featured_image = thrive_get_post_featured_image_src( $postId, "tt_post_icon" );
