...
/**
 * FocusBlog theme functions and definitions.
 *
...
function thrive_enqueue_scripts() {
	...
	wp_enqueue_style( 'pressive-style', get_stylesheet_uri() );
...
function thrive_init_widgets() {
	...
	register_sidebar( array(
		...
		'id'            => 'footer-1',
		'before_widget' => '<section id="%1$s" class="col"><div class="scn">',
		'after_widget'  => '</div></section>',
		'before_title'  => '<p class="ttl">',
		'after_title'   => '</p>',
	) );
...
require_once get_template_directory() . '/inc/helpers/social.php';
...
function thrive_enqueue_admin() {
	...
	if ( $screen->base == "post" || $screen->id == 'tcb_lightbox' && ! is_preview() ) {
	...
	if ( $screen->id == 'nav-menus' ) {
		wp_enqueue_media();
		wp_enqueue_script( 'admin-menu', get_template_directory_uri() . '/inc/js/admin-menu.js', array(
			'jquery',
			'media-upload',
			'thickbox'
		) );
	...
	if ( $screen->id == "thrive-dashboard_page_thrive_admin_options" || $screen->id == "focus_area" ) {
...
function thrive_create_post_types() {
	...
	register_post_type( 'thrive_optin', array(
		'labels'              => array(
			'name'          => __( 'Thrive Opt-In' ),
			'singular_name' => __( 'OptIn' )
		),
		'public'              => false,
		'show_ui'             => true,
		'exclude_from_search' => true,
...
function thrive_get_single_templates( $single_template ) {
	$post_type = get_post_type();
	if ( $post_type == TT_APPR_POST_TYPE_LESSON || $post_type == TT_APPR_POST_TYPE_PAGE ) {
	...
	if ( $template_page_name !== null ) {
		$single_template = TEMPLATEPATH . '/' . $template_page_name;
	} elseif ( is_single() ) {
		$default_blog_post_layout = thrive_get_theme_options( "blog_post_layout" );
		if ( $default_blog_post_layout == "full_width" ) {
			$single_template = TEMPLATEPATH . "/fullwidth-page.php";
		} elseif ( $default_blog_post_layout == "narrow" ) {
			$single_template = TEMPLATEPATH . "/narrow-page.php";
		}
	}
...
add_filter( 'single_template', 'thrive_get_single_templates' );
