...
function thrive_shortcode_appr_lessons_gallery( $attr, $content ) {
	...
	$cats = get_term_children( $attr['category'], 'apprentice' );
	if ( $attr['category'] != 0 ) {
		$attr['category'] = empty( $cats ) ? array( $attr['category'] ) : $cats;
	}
	...
	foreach ( $posts as $p ) {
		...
		$output .= "<a href='" . get_permalink( $p->ID ) . "'><h5>" . get_the_title( $p->ID ) . "</h5><p>" . _thrive_get_post_text_content_excerpt( $p->post_content, $p->ID, 120, array(), true ) . "</p></a>";
...
function thrive_shortcode_appr_welcome_back( $attr, $content ) {
	...
	if ( $lesson_link === false ) {
		...
		usort( $lessonsOrder, "_thrive_sort_by_order" );
