...
function thrive_appr_add_menu_page() {
	add_menu_page( "Manage Content", "Manage Content", "edit_theme_options", "thrive_admin_page_appr_contents", "thrive_admin_page_appr_contents", "", '59.52' );
}
...
function thrive_appr_get_theme_options() {
	...
	if ( isset( $theme_options['appr_different_logo'] ) && $theme_options['appr_different_logo'] == 1 && isset( $theme_options['appr_logo'] ) && ! empty( $theme_options['appr_logo'] ) ) {
		...
		$theme_options['logo_dark'] = $theme_options['appr_logo'];
	}
	...
	if ( isset( $theme_options['appr_featured_title_bg_type'] ) ) {
		$theme_options['featured_title_bg_type'] = $theme_options['appr_featured_title_bg_type'];
	}

	if ( isset( $theme_options['appr_featured_title_bg_solid_color'] ) ) {
		$theme_options['featured_title_bg_solid_color'] = $theme_options['appr_featured_title_bg_solid_color'];
	}

	if ( isset( $theme_options['appr_featured_title_bg_img_trans'] ) ) {
		$theme_options['featured_title_bg_img_trans'] = $theme_options['appr_featured_title_bg_img_trans'];
	}

	if ( isset( $theme_options['appr_featured_title_bg_img_default_src'] ) ) {
		$theme_options['featured_title_bg_img_default_src'] = $theme_options['appr_featured_title_bg_img_default_src'];
	}

	if ( isset( $theme_options['appr_featured_title_bg_color_type'] ) ) {
		$theme_options['featured_title_bg_color_type'] = $theme_options['appr_featured_title_bg_color_type'];
	}
...
function thrive_appr_enqueue_scripts() {
	...
	$params_array = array(
		...
		'lang'            => array(
			'remove_from_fav' => __( "Remove from Favorites", 'thrive' ),
			'add_to_fav'      => __( "Mark as Favorite", 'thrive' )
		),
		'progress_status' => array(
			'new'       => THRIVE_APPR_PROGRESS_NEW,
...
function thrive_appr_create_post_types() {
	...
	register_taxonomy( 'apprentice', array( TT_APPR_POST_TYPE_LESSON ), array(
		'hierarchical'      => true,
		'labels'            => array(
			'name'              => __( "Apprentice Category", 'thrive' ),
	...
	register_taxonomy( 'apprentice-tag', array( TT_APPR_POST_TYPE_LESSON ), array(
		'hierarchical'      => false,
		'labels'            => array(
			'name'              => __( "Apprentice Tags", 'thrive' ),
...
function thrive_admin_page_appr_contents() {
	...
	$queryLessonCatsPosts = new WP_Query( array(
		'post_type'      => TT_APPR_POST_TYPE_LESSON,
		'post_status'    => 'publish',
		'posts_per_page' => - 1
	) );
...
function thrive_appr_breadcrumbs() {
	...
	if ( $root_page_id != 0 && $homepage_url = get_permalink( $root_page_id ) ) {
...
if ( function_exists( 'pmpro_page_meta' ) ) {
	...
	/**
	 * Add the PMPro meta box to Apprentice Lessons and Pages
	 */
	function thrive_apprentice_meta_wrapper() {

		add_meta_box( 'pmpro_page_meta', 'Require Membership', 'pmpro_page_meta', 'appr_page', 'side' );
		add_meta_box( 'pmpro_page_meta', 'Require Membership', 'pmpro_page_meta', 'appr_lesson', 'side' );
	}

	function thrive_pmpro_init() {

		if ( is_admin() ) {
			add_action( 'admin_menu', 'thrive_apprentice_meta_wrapper' );
		}
	}

	add_action( 'init', 'thrive_pmpro_init', 20 );
