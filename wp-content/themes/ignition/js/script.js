...
ThriveApp.showMenu = function () {
	jQuery( 'header nav ul li' ).on( 'mouseenter', function () {
		var $this = jQuery( this ),
			_submenu_element = jQuery( this ).children( '.sub-menu' );

		if ( $this.hasClass( 'has-extended' ) && ThriveApp.winWidth >= 774 ) {
			var _main_item_position = $this.offset().left,
			...
			_submenu_element.css( 'max-height', jQuery( window ).height() - 100 ).css( 'overflow-y', 'auto' );
			_submenu_element.stop().fadeIn( 'fast' );
			ThriveApp.extended_menu_opened = true;
		} else {
			_submenu_element.show();
		}
	} ).on( 'mouseleave', function () {
		var _submenu_hidden = jQuery( this );
		_submenu_hidden.children( '.sub-menu' ).stop().fadeOut( 'fast' );
		if ( _submenu_hidden.hasClass( 'has-extended' ) ) {
			ThriveApp.extended_menu_opened = false;
		}
	} );
};
...
jQuery( window ).scroll( function () {
	...
	_hasFloatingMenu && ! ThriveApp.extended_menu_opened && ThriveApp.menu_float.onScroll( position );
	...
	return this.each( function () {
		...
		var htmlSetup = function () {
			...
			setValues( $dayContainer.css( 'min-width', (
				                                           el.find( '.cdtc.second .cdfc > span' ).outerWidth() * day_digits
			                                           ) + 'px' ), day, day_digits );
		};
